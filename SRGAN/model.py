...
def get_G(input_shape):
    ...
    n = Conv2d(64, (3, 3), (1, 1), act=tf.nn.lrelu, padding='SAME', W_init=w_init)(nin)
    temp = n

    # B residual blocks
    for i in range(16):
        nn = Conv2d(64, (3, 3), (1, 1), padding='SAME', W_init=w_init, b_init=None)(n)
        nn = BatchNorm2d(act=tf.nn.lrelu, gamma_init=g_init)(nn)
    ...
    n = SubpixelConv2d(scale=4, n_out_channels=None, act=tf.nn.lrelu)(n)
...
def get_D(input_shape):
    ...
    df_dim = 64
    leaky_relu = lambda x: tl.act.LeakyRelu(x, 0.2)

    nin = Input(input_shape)
    n = Conv2d(df_dim, (4, 4), (2, 2), act=leaky_relu, padding='SAME', W_init=w_init)(nin)

    n = Conv2d(df_dim * 2, (4, 4), (2, 2), padding='SAME', W_init=w_init, b_init=None)(n)
    n = BatchNorm2d(act=leaky_relu, gamma_init=gamma_init)(n)
    ...
    n = Elementwise(combine_fn=tf.add, act=leaky_relu)([n, nn])
